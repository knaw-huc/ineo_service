...
class Index:
    ...
    def browse(self, page, length, search_values):
        ...
        response = self.client.search(index=self.config["index"], body={
            "query": query,
            "size": length,
            "from": start,
        ...
        return {"amount": response["hits"]["total"]["value"],
                ...
                "items": [{"key": item["_id"], "title": item["_source"]["title"], "intro": item["_source"].get("intro", None), "tags": item["_source"]["properties"].get("resourceTypes", None)} for item in response["hits"]["hits"]]}
